export default function StatsCards({ data }) {
  ...
  const totalRechargedAll = rechargeHistory.reduce((s, r) => s + r.rechargeAmount, 0)
  const totalKwhAll = monthlyUsage.reduce((s, m) => s + m.usedKwh, 0)
  ...
  const cards = [
    ...
    },
    {
      ...
      labelBn: 'গড় মাসিক খরচ',
      value: `৳${avgMonthly.toFixed(0)}`,
      sub: `across ${monthlyUsage.length} months`,
      icon: (
        <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
